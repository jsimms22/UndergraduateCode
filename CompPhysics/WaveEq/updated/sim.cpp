...
int main() 
{
    ...
    constexpr double x_side {100.0};             // length of our 2d mesh sides
    constexpr double y_side {50.0};
    ...
    auto z_ptr = std::make_unique<Matrix3<double,nx,ny,n_steps>>();
    auto vz_ptr = std::make_unique<Matrix3<double,nx,ny,n_steps>>();
    ...
    for(size_t t_step = 0; t_step < n_steps; ++t_step) {
        ...
        if ((t_step % 5) == 0) { manifold::plot(z_ptr, dx, dy, {x_side, y_side}, t_step); }
    }
    return 0;
}
