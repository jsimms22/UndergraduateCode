...
namespace manifold
{
    ...
    constexpr double radius {0.5};                                  // radius of manifold
               ...
               const UNIT dx, const UNIT dy)
    {
        ...
        for(int i = 1; i < NX - 1; i++) {
            for(int j = 1; j < NY -1; j++) {
                ...
                // (*accel)[i][j] -= 0.5 * (c * c * (dx2 + dy2)); // dampening
            }
        }
    }
    ...
    // calculate acceleration components for a given time step
    void accel(const std::unique_ptr<matrix::Matrix2<UNIT,NX,NY>>& pos, 
               const std::unique_ptr<matrix::Matrix2<UNIT,NX,NY>>& accel,
               const UNIT dx, const UNIT dy)
    {
        double dx2, dy2;
        for(int i = 1; i < NX - 1; i++) {
            for(int j = 1; j < NY -1; j++) {
                dx2 = ((*pos)[i+1][j] - 2 * (*pos)[i][j] + (*pos)[i-1][j]) / (dx * dx);
                dy2 = ((*pos)[i][j+1] - 2 * (*pos)[i][j] + (*pos)[i][j-1]) / (dy * dy);
                (*accel)[i][j] = c * c * (dx2 + dy2);
                // (*accel)[i][j] -= 0.5 * (c * c * (dx2 + dy2)); // dampening
            }
        }
    ...
    void plot(const std::unique_ptr<matrix::Matrix3<UNIT,NX,NY,NT>>& z_matrix, 
              const double dx, const size_t t_step, const double side)
    {
        // initialize our mesh grid matrices, matplot++ restricts us to doubles
        auto [X,Y] = matplot::meshgrid(matplot::iota(0.0,dx,(side+dx)));
